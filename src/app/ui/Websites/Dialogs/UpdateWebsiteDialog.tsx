...
export default function UpdateWebsiteDialog({
    ...
    const [dataSourcesToPull, setDataSourcesToPull] = useState<string[]>([
        'monit_site_audit',
        'monit_security_review'
    ]);
    const [showSourcesToPull, setShowSourcesToPull] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        async function getData() {
            ...
            if (loadedWebsite.type.name === 'Drupal') {
                setShowSourcesToPull(true)
                setDataSourcesToPull(loadedWebsite.dataSourcesToPull?.length ? loadedWebsite.dataSourcesToPull : [
                    'monit_site_audit',
                    'monit_security_review'
                ]);
            }
        ...
        open && websiteId && getData().finally(() => {
            setIsLoading(false);
        });
        setIsSaving(false);
    }, [websiteId, open]);
    ...
    return (
        <div>
                                    ...
                                    <Grid size={12}>
                                        <FormControlLabel
                                            ...
                                            control={<Checkbox
                                                id={"none"}
                                                name={"none"}
                                                checked={dataSourcesToPull.includes('none')} onChange={
                                                (e) => {
                                                    if (e.target.checked) {
                                                        setDataSourcesToPull(['none']);
                                                    } else if (!e.target.checked) {
                                                        setDataSourcesToPull(dataSourcesToPull.filter((item) => item !== 'none'));
                                                    }
                                                }
                                            }/>}
                                        />
                                            ...
                                            control={<Checkbox
                                                ...
                                                (e) => {
                                                    if (!dataSourcesToPull.includes('monit_site_audit') && e.target.checked) {
                                                        setDataSourcesToPull([...dataSourcesToPull.filter((item) => item !== 'none'), 'monit_site_audit']);
                                                    } else if (!e.target.checked) {
                                            ...
                                            control={<Checkbox
                                                ...
                                                (e) => {
                                                    if (!dataSourcesToPull.includes('monit_security_review') && e.target.checked) {
                                                        setDataSourcesToPull([...dataSourcesToPull.filter((item) => item !== 'none'), 'monit_security_review']);
                                                    } else if (!e.target.checked) {
                        ...
                        <Grid container spacing={2}>
                            <Grid size={12}>
                                <FormControlLabel
                                    ...
                                    control={<Checkbox checked={enableSync} onChange={(e) => {
                            ...
                            {enableSync && (
                                <>
                                    ...
                                    <Grid size={12}>
                                        {timeZone ? (
                                            <Typography variant={'caption'}>Timezone: {timeZone} (Timezone can be
                                                changed from <Link href={"/settings"}>workspace
                                                    setting</Link>)</Typography>
                                        ) : (
                                            <Typography variant={'caption'}>Timezone is not available in personal
                                                workspaces we use UTC instead</Typography>
                                        )}
                                    </Grid>
                                </>
                            )}
                            ...
                            onClick={() => {
                                ...
                                async function save() {
                                    await updateWebsite(websiteId, {
                                        ...
                                        dataSourcesToPull: dataSourcesToPull.length ? dataSourcesToPull : [],
                                        syncConfig: {
                                            enabled: enableSync,
                                            syncInterval,
            ...
            {isDeleteOpen && (
                <DeleteWebsiteModal open={isDeleteOpen} setOpen={(isOpen) => handleDeleteOpen(isOpen, setIsDeleteOpen)}
                                    websiteId={websiteId}></DeleteWebsiteModal>
            )}
        </div>
    );
}