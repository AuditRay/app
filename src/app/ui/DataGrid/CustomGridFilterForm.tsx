...
import {
    ...
    GridColDef, gridColumnLookupSelector, gridFilterableColumnDefinitionsSelector, gridFilterModelSelector,
    GridLogicOperator, GridSingleSelectColDef,
    useGridApiContext, useGridRootProps,
    useGridSelector
} from '@mui/x-data-grid';
import {GridFilterItem} from "@mui/x-data-grid-pro";
import {DataGridProcessedProps, GridStateColDef} from '@mui/x-data-grid/internals';
import {forwardRef} from "react";
import {
    getValueFromValueOptions,
    getValueOptions
} from "@mui/x-data-grid/components/panel/filterPanel/filterPanelUtils";
import {Autocomplete, AutocompleteChangeDetails, autocompleteClasses, Box} from "@mui/material";
...
const getValueById = (key: string, options: GridStateColDef<any, any, any>[]) => {
    const option = options.find((opt) => opt.field === key);
    return option ? {
        label: option.field,
        headerName: option.headerName ?? option.field,
    } : {
        label: '',
        headerName: '',
    };
...
const CustomGridFilterForm = forwardRef<HTMLDivElement, GridFilterFormProps>(
    function CustomGridFilterForm(props, ref) {
        const {
            item,
            hasMultipleFilters,
            deleteFilter,
        ...
        const changeColumn = React.useCallback(
            (event: React.SyntheticEvent, value: {label: string; headerName: string; } | null, reason: string, details?: AutocompleteChangeDetails<{label: string; headerName: string; }>) => {
                const field = value?.label as string;
        ...
        return (
            ...
            >
                ...
                >
                    ...
                    <Autocomplete
                        id={columnSelectId}
                        value={getValueById(selectedField, sortedFilteredColumns)}
                        onChange={changeColumn}
                        groupBy={(option) => option.headerName.includes("-") ? option.headerName.split("-")[0].trim() : 'Website Info'}
                        disableClearable={true}
                        options={sortedFilteredColumns.map((col) => ({
                            label: col.field,
                            headerName: col.headerName ?? col.field,
                        }))}
                        getOptionLabel={(option) => option.headerName.split("-").length > 1 ? option.headerName.split("-")[1].trim() : option.headerName.split("-")[0].trim()}
                        getOptionKey={(option) => option.label}
                        renderInput={(params) => <TextField label= {apiRef.current.getLocaleText('filterPanelColumns')} variant={'standard'} {...params} />}
                    />
                    {/*<rootProps.slots.baseSelect*/}
...
CustomGridFilterForm.propTypes = {
    // ----------------------------- Warning --------------------------------
